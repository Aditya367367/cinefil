        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {loading || minimumLoading ? (
            <div className="flex flex-wrap gap-8 justify-center mt-12">
              {[...Array(4)].map((_, index) => (
                <div key={index} className="flex flex-col items-center justify-between p-6 bg-white border border-slate-200 rounded-2xl w-64 text-center shadow-xs min-h-[340px]">
                  <div className="w-40 h-40 rounded-2xl bg-slate-200 animate-pulse" />
            ...
            <div className="flex flex-wrap gap-8 justify-center mt-12">
              {honoraryMembers.map((member, i) => (
                <div 
                  key={`${member.name}-${i}`} 
                  className="flex flex-col items-center justify-between p-6 bg-white border border-slate-200 rounded-2xl w-64 text-center shadow-xs hover:shadow-md hover:-translate-y-1 transition-all duration-300 group min-h-[340px]"
                >
                  <div className="flex flex-col items-center w-full">
                    {/* Frame Container Shell */}
                    <div
                      className="w-40 h-40 rounded-2xl overflow-hidden flex items-center justify-center bg-gradient-to-b from-slate-50 to-slate-100 p-0.5 group-hover:scale-102 transition-transform duration-300 shadow-inner relative"
