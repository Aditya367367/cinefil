          ...
          {loading || minimumLoading ? (
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-x-5 gap-y-8 mt-10">
              {[...Array(10)].map((_, index) => (
                <div key={index} className="flex flex-col items-center text-center bg-white border border-slate-100 rounded-2xl p-4 shadow-xs min-h-[260px]">
                  <div className="w-full aspect-square rounded-xl bg-slate-200 animate-pulse" />
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-x-5 gap-y-8 mt-10">
              {boardMembers.map((member, i) => (
                <div 
                  key={`${member.name}-${i}`} 
                  className="flex flex-col items-center text-center bg-white border border-slate-100 rounded-2xl p-4 shadow-xs hover:shadow-md hover:-translate-y-1 transition-all duration-300 group justify-between min-h-[260px]"
                >
                  <div className="flex flex-col items-center w-full">
                    {/* Professional Profile Picture Shell */}
                    <div
                      className="w-full aspect-square rounded-xl overflow-hidden flex items-center justify-center bg-gradient-to-b from-slate-50 to-slate-100 p-0.5 group-hover:scale-102 transition-transform duration-300 shadow-inner relative"
