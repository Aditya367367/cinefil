      ...
      <section className="py-16 sm:py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-10">
            <div>
              <span className="text-[11px] font-bold tracking-widest uppercase text-[var(--cinefil-gold)] bg-amber-50 border border-amber-200 px-3 py-1 rounded-full">
                Verified Directory
              </span>
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
              <div className="relative flex-1 sm:w-72">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-4 py-2.5 bg-slate-50 border border-gray-200 rounded-xl text-xs font-semibold text-gray-800 placeholder:text-gray-400 focus:outline-none focus:border-[var(--cinefil-gold)] focus:ring-2 focus:ring-[var(--cinefil-gold)]/20 transition shadow-sm"
                />
              </div>
              ...
              <div className="flex bg-slate-100 p-1 rounded-xl border border-gray-200">
                <button
                  ...
                  className={`px-3.5 py-1.5 text-xs font-bold rounded-lg cursor-pointer transition-all ${
                    filterTab === "all" ? "bg-[var(--cinefil-navy)] text-white shadow-sm" : "text-gray-600 hover:text-gray-900"
                  }`}
                >
                  All ({members.length})
                </button>
                <button
                  ...
                  className={`px-3.5 py-1.5 text-xs font-bold rounded-lg cursor-pointer transition-all ${
                    filterTab === "prime" ? "bg-[var(--cinefil-navy)] text-white shadow-sm" : "text-gray-600 hover:text-gray-900"
                  }`}
                >
              ...
              <div className="flex items-center bg-slate-100 p-1 rounded-xl border border-gray-200">
                <button
                  ...
                  className={`p-2 rounded-lg cursor-pointer transition-all ${
                    viewMode === "grid" ? "bg-white text-[var(--cinefil-navy)] shadow-sm" : "text-gray-500 hover:text-gray-900"
                  }`}
                >
                  <LayoutGrid size={16} />
                </button>
                <button
                  ...
                  title="Table View"
                  className={`p-2 rounded-lg cursor-pointer transition-all ${
                    viewMode === "table" ? "bg-white text-[var(--cinefil-navy)] shadow-sm" : "text-gray-500 hover:text-gray-900"
                  }`}
                >
                  <TableIcon size={16} />
                </button>
          ...
          {loading ? (
            ...
            <div className="bg-slate-50 rounded-3xl border border-dashed border-gray-300 p-12 text-center max-w-xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {paginatedMembers.map((m) => {
                ...
                return (
                  <SpotlightCard
                    key={m.id}
                    className="p-6 rounded-3xl border-gray-200 shadow-lg hover:shadow-2xl hover:-translate-y-1.5 transition-all duration-300 flex flex-col justify-between group bg-white"
                  ...
                  >
                    <div>
                      <div className="flex items-start gap-4 mb-4">
                        {m.photo_url ? (
                          <img
                            src={m.photo_url}
                            alt={m.full_name}
                            className="w-16 h-16 rounded-2xl object-cover border-2 border-[var(--cinefil-gold)] shadow-md group-hover:scale-105 transition-transform"
                          />
                        ) : (
                          <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-[#0f2540] to-[#183858] text-[var(--cinefil-gold)] flex items-center justify-center font-black text-xl border-2 border-[var(--cinefil-gold)] shadow-md">
                      ...
                      <div className="flex items-center gap-2 mb-4">
                        <span className="inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider text-emerald-700 bg-emerald-50 border border-emerald-200 px-2.5 py-1 rounded-lg">
                          <ShieldCheck size={12} /> Cinefil Verified
                        </span>
                        <span className="text-[10px] font-bold uppercase tracking-wider text-gray-600 bg-slate-100 border border-gray-200 px-2.5 py-1 rounded-lg">
                          {m.membership_type_name || m.role_title || "Listed Member"}
                        </span>
                      </div>

                      <div className="text-xs text-gray-600 space-y-1.5 py-2.5 border-t border-gray-100 mb-4 bg-slate-50 px-3 rounded-xl">
                    ...
                    <button
                      ...
                      className="w-full mt-2 inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-[var(--cinefil-navy)] hover:bg-[var(--cinefil-navy-mid)] text-white font-bold text-xs transition-all shadow-md cursor-pointer transform hover:scale-[1.02] active:scale-95"
                    >
                    ...
                    return (
                      <tr key={m.id} className="hover:bg-amber-50/40 transition-colors">
                        ...
                        <td className="py-4 px-5">
                          <span className="px-2.5 py-1 rounded-md bg-slate-100 text-gray-700 font-bold text-[10px] uppercase tracking-wider border border-gray-200">
                        ...
                        <td className="py-4 px-5 text-right">
                          <button
                            onClick={() => onNavigate(`profile/${slug}`)}
                            className="px-3.5 py-1.5 rounded-lg bg-[var(--cinefil-navy)] text-white text-xs font-bold hover:bg-[var(--cinefil-navy-mid)] transition-colors cursor-pointer"
                          >
                            Profile
                          </button>
                        </td>
          ...
          {/* Pagination Controls */}
          {totalPages > 1 && (
            <div className="mt-8 flex items-center justify-between bg-slate-50 p-4 rounded-2xl border border-gray-200">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="px-3.5 py-1.5 rounded-xl border border-gray-200 bg-white text-gray-700 text-xs font-bold disabled:opacity-40 cursor-pointer hover:bg-gray-100 transition-colors"
                >
