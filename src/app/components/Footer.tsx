...
export function Footer({ onNavigate }: FooterProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.id) {
        setSuccess("Thank you! Your enquiry was submitted successfully.");
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 pb-10 border-b border-white/10">
          ...
          <div>
            ...
            {success && (
              <div className="mb-3 p-2 text-xs bg-emerald-500/20 border border-emerald-500/40 text-emerald-200 rounded-lg">
                {success}
              </div>
            )}
            {error && (
              <div className="mb-3 p-2 text-xs bg-rose-500/20 border border-rose-500/40 text-rose-200 rounded-lg">
                {error}
              </div>
            )}
